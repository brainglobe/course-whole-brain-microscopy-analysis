...
![](img/ml_augmentation.png){.nostretch .absolute top="24%" left="6%" width="88%"}

::: {.notes}
Intensity, noise and blur stand in for real acquisition variation - different
sessions, different scopes, different laser power. This is the same idea
Cellpose-SAM leans on later in the deck, where the model is deliberately
trained against blur, noise and anisotropy.

For segmentation the mask has to go through the same transform as the image;
flipping one and not the other is the classic bug.
:::

## Augment the training set only {.smaller}
...
## [ilastik](https://www.ilastik.org/) {.smaller}
...
## [StarDist](https://stardist.net/) {.smaller}
...
## [Cellpose 2](https://cellpose.readthedocs.io/en/latest/index.html) {.smaller}
...
## [Segment Anything (SAM)](https://segment-anything.metademolab.com/) {.smaller}
...
## [Cellpose-SAM](https://cellpose.readthedocs.io/en/latest/index.html) {.smaller}
